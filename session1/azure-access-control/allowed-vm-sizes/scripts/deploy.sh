...
if az policy definition show --name "$POLICY_NAME" >/dev/null 2>&1; then
  ...
  az policy definition update \
    --name "$POLICY_NAME" \
    --rules @azure-access-control/allowed-vm-sizes/definition/policy.json \
    --mode Indexed \
...
az deployment sub create \
  --location "$LOCATION" \
  --template-file azure-access-control/allowed-vm-sizes/assignment/assign.bicep \
  --parameters @azure-access-control/allowed-vm-sizes/assignment/assign-allowed-vms.parameters.json \
